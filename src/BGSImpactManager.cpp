...
bool BGSImpactManager::PlayImpactEffect(RE::TESObjectREFR* a_subject, const Settings::PlayImpactEffect& a_playImpactEffect)
{
	...
	else if (rayCast.rotation.x || rayCast.rotation.y || rayCast.rotation.z)
	{
		...
		auto angle = a_subject->GetAngle();
		rotation.EulerAnglesToAxesZXY(rayCast.rotation.x ? angle.x : 0.0F, rayCast.rotation.y ? angle.y : 0.0F, rayCast.rotation.z ? angle.z : 0.0F);
	...
	offset.Unitize();
	offset *= rayCast.origin.offset.length;
	origin += offset;

	ray.Unitize();
	ray *= rayCast.ray.length;
		...
		if (rootCollidable)
		{
			...
			if (terrain)
			{
				...
				if (rootCollidable->broadPhaseHandle.type == 1)
				{
					...
					if (owner)
					{
						auto userData = reinterpret_cast<RE::bhkWorldObject*>(owner->userData);

						if (userData)
						{
							auto propertyValue = userData->GetPropertyValue<RE::NiAVObject>(1);
					...
					if (impactData)
					{
						...
						if (textureSet)
						{
							...
							if (terrain || (target && RE::BGSDecalManager::ShouldApplyDecal(target.get())))
							{
								...
								{
									decalCreationData.rotation = decalRotationNode->world.rotate;
								}
								else if (decal.rotation.x || decal.rotation.y || decal.rotation.z)
								{
									auto angle = a_subject->GetAngle();
									decalCreationData.rotation.EulerAnglesToAxesZXY(decal.rotation.x ? angle.x : 0.0F, decal.rotation.y ? angle.y : 0.0F, decal.rotation.z ? angle.z : 0.0F);
								}
